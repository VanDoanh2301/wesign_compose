...
package com.example.camera_ai.util.tracking;
...
public class MultiBoxTrackerPractice {
  ...
  private int initPractice;

  public MultiBoxTrackerPractice(final Context context, final Integer initPractice) {
    this.initPractice = initPractice;
  ...
  public synchronized void draw(final Canvas canvas, CallTittle callTittle) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if (recognition.detectionConfidence >= 0 && recognition.detectionConfidence <= 0.15) {
        result = "Sai";
      } else if (recognition.detectionConfidence > 0.15 && recognition.detectionConfidence <= 0.3) {
        result = "Gần đúng";
      } else {
        result = "Đúng";
      }
      if (initPractice == 0) {
        final String labelString =
                !TextUtils.isEmpty(recognition.title)
                        ? String.format("%s - %s","Kết quả: " + recognition.title, result)
